...
public class BoardGame {
    ...
    public BoardGame(boolean fill) {
        for (GridCell[] gameCells : map) { Arrays.fill(gameCells, GridCell.EMPTY); }
        if (fill) { buildMap(); }
    ...
    private boolean canFit(int length, int x, int y, BoatPositioning Position) {
        if (x >= getWidth() || y >= getHeight() || getCell(x, y) != GridCell.EMPTY) return false;
    ...
    public GridCell getCell(int x, int y) {
        if (x >= 10 || y >= 10) throw new RuntimeException("Invalidate coordinates!");
        return map[x][y];
    }
    public void setCell(Hook coordinates, GridCell newStatus) { map[coordinates.getX()][coordinates.getY()] = newStatus;
    ...
    public void addBoat(int length, int x, int y, BoatPositioning orientation) {
        ...
        while (length > 0) { map[x][y] = GridCell.BOAT;length--;coordinates.add(new Hook(x, y));
            switch (orientation) { case HORIZONTAL -> x++;case VERTICAL -> y++; }
        }
        boats.add(coordinates);
    }
    public boolean hasShipLeft() {
        for (var row : map) { if (Arrays.stream(row).anyMatch(s -> s == GridCell.BOAT)) return true; }
        return false;
    }
    ...
    private Hook fireAroundSuccessfulHit() { while (positionsToTest.size() > 0) { var c = positionsToTest.remove(0);if (getCell(c) == GridCell.EMPTY) return c; }
        return null;
    }
